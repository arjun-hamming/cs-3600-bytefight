...
class PlayerController:
    ...
    def play(
        ...
        for our_value, our_actions, our_board in candidates[: self.TOP_CANDIDATES]:
            ...
            reply_deadline = min(deadline, time.perf_counter() + max(0.015, budget * 0.18))
    ...
    def _time_budget(self, board: Board, time_left: Callable) -> float:
        try:
            ...
            remaining = 15.0
        area = len(board.cells) * len(board.cells[0])
        remaining_rounds = max(1, GameConstants.MAX_ROUNDS - board.current_round)
        average_slice = remaining / remaining_rounds
        map_penalty = 1.0 if area <= 121 else 0.7 if area <= 196 else 0.55
        budget = 0.45 * average_slice * map_penalty
        if remaining < 30:
            budget *= 0.75
        return min(0.18, max(0.03, budget))
    ...
    def _powerup_score(self, board: Board, player_parity: int) -> float:
        ...
        for r, row in enumerate(board.cells):
            for c, cell in enumerate(row):
                if cell.powerup:
                    dist = abs(player.loc.r - r) + abs(player.loc.c - c)
                    score += 10.0 / (1.0 + dist)
        return score
